...
public class ftLightmaps {
    ...
    public static ftGlobalStorage GetGlobalStorage()
    {
        ...
        {
            var gstorageDefault = AssetDatabase.LoadAssetAtPath("Assets/Bakery/ftDefaultGlobalStorage.asset", typeof(ftGlobalStorage)) as ftGlobalStorage;

            if (gstorageDefault != null)
            {
                if (AssetDatabase.CopyAsset("Assets/Bakery/ftDefaultGlobalStorage.asset", "Assets/Bakery/ftGlobalStorage.asset"))
                {
            ...
            if (gstorage == null)
            {
                ...
                AssetDatabase.CreateAsset(gstorage, "Assets/Bakery/ftGlobalStorage.asset");
...
/*    public static void RefreshScene(int sceneID, ref List<LightmapData> lmaps, int lmCounter) {
    ...
    public static void RefreshScene(Scene scene, ftLightmapsStorage storage = null, bool updateNonBaked = false) {
        ...
        bool directionalMode = storage.dirMaps.Count != 0;
        ...
        */
        ...
        if (RenderSettings.ambientMode == UnityEngine.Rendering.AmbientMode.Skybox)// && Lightmapping.lightingDataAsset == null)
        {
            ...
            for(int i=0; i<3; i++)
            {
                for(int j=0; j<9; j++)
                {
                    // default bugged probes are black or 1302?
