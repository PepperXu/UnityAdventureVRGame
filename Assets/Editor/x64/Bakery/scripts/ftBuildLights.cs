    ...
    static public bool BuildLight(BakeryPointLight obj, int SAMPLES, bool texDirty, bool ignoreNormal = false, string outName = "pointlight.bin")
    {
        ...
        else if (isIES)
        {
            if (texDirty)
            {
                ...
                {
                    ftRenderLightmap.DebugLogError("ies2tex error: "+ ftErrorCodes.TranslateI2T(exeProcess.ExitCode));
                    Debug.LogError("ies2tex error: "+ ftErrorCodes.TranslateI2T(exeProcess.ExitCode) + " with args " + startInfo.Arguments);
                    isError = true;
                }
            }
        }
