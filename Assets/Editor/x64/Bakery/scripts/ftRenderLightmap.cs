...
public class ftRenderLightmap : EditorWindow//ScriptableWizard
{
            ...
            if (errCode == 0 && errCode2 == 0 && errCode3 == 0 && errCode4 == 0 && errCode5 == 0 && errCode6 == 0)
            {
                for(int i=0; i<storage.bakedIDs.Count; i++)
                {
                    ...
                    */
                    ...
                    AssetDatabase.CreateAsset(newMesh, "Assets/" + outputPath + "/" + lmname + i + ".asset");
                }
    ...
    void OnGUI()
    {
        ...
        GUI.Label(aboutRect, new GUIContent("<color=#5073c9ff><b>Bakery - GPU Lightmapper by Mr F</b></color>", "Version 1.45. Go to author's site"), linkStyle);
        ...
        y += 15;
        //GUI.Label(new Rect(198, y+7, 50, 20), "<color=#5073c9ff><b>v1.45</b></color>", linkStyle);

        bool simpleWindowIsTooSmall = position.height < 300;

        if (settingsMode >= SettingsMode.Advanced || simpleWindowIsTooSmall)
        {
            float scrollHeight = 700+y+(showAOSettings ? 65 : 15)+(showTasks ? 160 : 0)+(showPaths ? 70 : 0) + (userRenderMode==RenderMode.Shadowmask ? 20 : 0) + 40;
            ...
            if (settingsMode == SettingsMode.Advanced) scrollHeight -= 60;
            if (settingsMode == SettingsMode.Simple) scrollHeight = this.minSize.y - 30;
            if (settingsMode == SettingsMode.Experimental) scrollHeight += 40;
        ...
        ftBuildGraphics.maxAutoResolution = 1 << (int)GUI.HorizontalSlider(new Rect(50, ay, 170, 15), Mathf.Log(ftBuildGraphics.maxAutoResolution)/Mathf.Log(2), 8, 12);
        ay += 20;
        ...
        if (settingsMode >= SettingsMode.Advanced)
        {
            ...
            //if (showHacks)
            {
                ...
                GUI.Label(new Rect(10, 20+yy, 100, 15), new GUIContent("Emissive boost:", "Multiplies light from emissive surfaces."));
                hackEmissiveBoost = EditorGUI.FloatField(new Rect(110, 20+yy, 110, 15), hackEmissiveBoost);

                GUI.Label(new Rect(10, 20+20+yy, 120, 20), new GUIContent("Backface GI:", "How much light is emitted via back faces from 0 (black) to 1 (equals to front face)."));
                giBackFaceWeight = EditorGUI.FloatField(new Rect(110, 20+20+yy, 110, 15), giBackFaceWeight);

                showAOSettings = EditorGUI.Foldout(new Rect(10, 60+yy, 300, 20), showAOSettings, "Ambient occlusion");
                y += 15+20;
                if (showAOSettings)
                {
                    int xx = 15;
            ...
            GUI.BeginGroup(new Rect(10, 10+y-10, 300, 120), "GI VRAM optimization", headerStyle);
            y += 45;
            GUI.EndGroup();
        }
        ...
        if (settingsMode >= SettingsMode.Advanced)
        {
            showPaths = EditorGUI.Foldout(new Rect(10, y, 230, 20), showPaths, "Temp and output path", foldoutStyle);
            y += 20;
            ...
            if (showPaths)
            {
                ...
                outputPath = EditorGUI.TextField(new Rect(85, y, 155, 15), outputPath);
                y += 25;
            }
        }
        ...
        for(int s=0; s<sceneCount; s++)
        {
            ...
            if (Lightmapping.lightingDataAsset == null)
            {
                if (emptyLDataAsset == null) emptyLDataAsset =
                    AssetDatabase.LoadAssetAtPath("Assets/Bakery/emptyLightingData.asset", typeof(LightingDataAsset)) as LightingDataAsset;
                ...
                if (copiedAsset == null)
                {
                    ...
                    if (AssetDatabase.CopyAsset("Assets/Bakery/emptyLightingData.asset",  outName))
                    {
    ...
    IEnumerator RenderLightProbesFunc()
    {
        ...
        if (!useUnityForOcclsusionProbes)
        {
            ...
            var tempScenePath = "Assets/Bakery/_tempScene.unity";
	...
	IEnumerator RenderLightmapFunc()
	{
        ...
        if (selectedOnly)
        {
            ...
            for(int i=0; i<groups.Count; i++)
            {
                ...
                if (!groupListGIContributing.Contains(lmgroup))
                {
                    ...
                    if (!File.Exists(outfile) && !groupList.Contains(lmgroup)) continue;
                    groupListGIContributing.Add(lmgroup);
                    groupListGIContributingPlain.Add(lmgroup.GetPlainStruct());
                }
            }
        ...
        */
        ...
        if (deferredMode)
        {
            ...
            for(int i=0; i<deferredCommands.Count; i++)
            {
                ...
                if (deferredCommandsRebake.ContainsKey(i))
                {
                    ...
                    Debug.Log("Running lmrebake for " + lmgroup2.name);
        ...
        foreach(var lmgroup in groupListGIContributingPlain)
        {
            ...
            //if (!bc6h)
            {
                //if (File.Exists(folder + "../Assets/" + nm + "_final.hdr"))
                {
                    ...
                    var desiredTextureType = encodeMode == 0 ? TextureImporterType.Lightmap : TextureImporterType.Default;
                    if (lightmapHasColor[lmgroup.id] && File.Exists(outfile))
                    {
                        importer = AssetImporter.GetAtPath(outfile) as TextureImporter;
                        if (importer.maxTextureSize != colorSize || importer.mipmapEnabled || importer.wrapMode != TextureWrapMode.Clamp ||
                            importer.textureType != desiredTextureType)
                        {
                            importer.textureType = desiredTextureType;
                            importer.maxTextureSize = colorSize;
                    ...
                    if (lightmapHasMask[lmgroup.id])
                    {
                        ...
                        {
                            desiredTextureType = TextureImporterType.Default;
                            importer = AssetImporter.GetAtPath(outfile) as TextureImporter;
                            if (importer.maxTextureSize != maskSize || importer.mipmapEnabled || importer.wrapMode != TextureWrapMode.Clamp ||
                                importer.textureType != desiredTextureType)
                            {
                                importer.textureType = desiredTextureType;
                    ...
                    if (dominantDirMode)
                    {
                        ...
                        desiredTextureType = TextureImporterType.Default;
                        importer = AssetImporter.GetAtPath(outfile) as TextureImporter;
                        if (importer.maxTextureSize != dirSize || importer.mipmapEnabled || importer.wrapMode != TextureWrapMode.Clamp ||
                            importer.textureType != desiredTextureType || importer.sRGBTexture)
                        {
                            importer.textureType = desiredTextureType;
                            importer.maxTextureSize = dirSize;
                            importer.mipmapEnabled = false;
                            importer.wrapMode = TextureWrapMode.Clamp;
                            importer.textureCompression = TextureImporterCompression.CompressedHQ;
                    ...
                    if (rnmMode)
                    {
                        desiredTextureType = encodeMode == 0 ? TextureImporterType.Lightmap : TextureImporterType.Default;
                        for(int c=0; c<3; c++)
                        {
            ...
            /*else
            {
                //if (File.Exists(folder + "../Assets/" + nm + "_final.asset"))
                {
                    //var outfile = "Assets/"+nm+"_final.hdr";
                    var outfile = "Assets/" + outputPath + "/"+nm+"_final.asset";

                    var lm = AssetDatabase.LoadAssetAtPath(outfile, typeof(Texture2D)) as Texture2D;
                    for(int s=0; s<sceneCount; s++)
                    {
                        var scene = EditorSceneManager.GetSceneAt(s);
                        if (!scene.isLoaded) continue;
        ...
        Debug.Log("Rendering finished in " + ((ms - startMs) / 1000.0) + " seconds");
    ...
    IEnumerator RenderLMDirect(int LMID, string lmname, int resolution)
    {
        ...
        for(int i=0; i<All.Length; i++)
        {
            ...
            ftBuildLights.BuildLight(obj, draftShadows? 1 : obj.samples, deferredMode ? ("lights" + i + ".bin") : "lights.bin");
            ...
            if (doCompose)
            {
                ...
                if (bounces > 0)
                {
                    fcomp.Write(obj.indirectIntensity);
    ...
    */
    ...
    IEnumerator RenderLMFinalize(int LMID, string lmname, int resolution, bool vertexBake, int lmgroupRenderDirMode)
    {
        ...
        if (userRenderMode == RenderMode.Shadowmask)
        {
            ...
            if (maskNames.Count > 0)
            {
                ...
                {
                    startInfo.Arguments       = "\"" + Application.dataPath + "/" + outputPath + "/" + lmname + "_mask.tga\" ";
                }
                                                                /*maskNames[0] + " ";
            ...
            if (!shMode && !rnmMode)
            {
                ...
                else
                {
                    ...
                    while(exeProcess.ExitCode >= 504 && exeProcess.ExitCode <= 506 && denoiseRes > 64)
                    {
                        ...
                        startInfo.Arguments = firstArgs + " " + denoiseRes;
                        Debug.Log("Running denoiser " + startInfo.Arguments);
                        exeProcess = System.Diagnostics.Process.Start(startInfo);
                        exeProcess.WaitForExit();
                    }
	...
	public static void RenderLightmap ()
    {
        ...
        instance.titleContent.image = EditorGUIUtility.Load("Assets/Editor/x64/Bakery/icon.png") as Texture2D;
        instance.Show();
	}
}
