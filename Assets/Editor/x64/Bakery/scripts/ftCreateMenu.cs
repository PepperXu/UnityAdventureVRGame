...
public class ftCreateMenu
{
    ...
    private static void CreateAreaLight()
    {
        ...
        var ecam = SceneView.lastActiveSceneView.camera.transform;
        go.transform.position = ecam.position + ecam.forward;
        var mat = AssetDatabase.LoadAssetAtPath("Assets/Bakery/ftDefaultAreaLightMat.mat", typeof(Material)) as Material;
    ...
    private static void CreateSpotLight()
    {
        ...
        light.cookie = AssetDatabase.LoadAssetAtPath("Assets/Bakery/ftUnitySpotTexture.bmp", typeof(Texture2D)) as Texture2D;
        go.name = "SpotLight";
        var ecam = SceneView.lastActiveSceneView.camera.transform;
        go.transform.position = ecam.position + ecam.forward;
        var arr = new GameObject[1];
